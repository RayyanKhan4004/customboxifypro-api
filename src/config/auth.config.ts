...
export class AuthConfig {
  ...
  constructor(private readonly config: ConfigService) {
    ...
    this.cookies = {
      ...
      refreshName:
        config.get<string>('COOKIE_REFRESH_NAME') ?? 'boxify_refresh',
      domain: config.get<string>('COOKIE_DOMAIN') || undefined,
      secure:
        config.get('COOKIE_SECURE') === true ||
        config.get<string>('COOKIE_SECURE') === 'true',
      sameSite: (config.get<string>('COOKIE_SAME_SITE') ??
        'lax') as CookieSettings['sameSite'],
    };
    ...
    this.passwordMinLength = Number(
      config.get('AUTH_PASSWORD_MIN_LENGTH') ?? 12,
    );
  }
}
